...
export class MailService {
  ...
  constructor(private config: ConfigService) {
    ...
    this.from = this.config.get<string>('EMAIL_FROM') ?? 'noreply@servizein.ma';

    if (!apiKey) {
      this.logger.warn('RESEND_API_KEY missing — email sending disabled');
      return;
    }
    ...
    try {
      ...
      this.logger.error('Resend init failed: ' + err.message);
    }
  }
  ...
  async sendBookingConfirmation(opts: {
    ...
    if (!this.resend) {
      this.logger.warn('Email not sent — Resend not initialized');
      return;
    }
